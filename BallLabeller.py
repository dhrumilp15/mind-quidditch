...
class BallLabeller:
    def __init__(self):
        ...
        filenames = [name[:name.index(".jpg")] for name in os.listdir(
            "ball_images/") if name.endswith(".jpg")]
        filenames.sort(key=lambda x: int(x))
        images = [cv2.imread("ball_images/" + img + ".jpg")
    ...
    def click_radius(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.center:
                self.radius = np.sqrt(
                    (y - self.center[1])**2 + (x - self.center[0])**2)
            else:
                self.center = (x, y)
