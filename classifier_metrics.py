...
class ClassifierMetrics:
    ...
    def get_img_iter(self, folder):
        ...
        filenames = [name for name in os.listdir(folder) if name.endswith(".jpg")]
        if not filenames:
    ...
    def generateReport(self):
        ...
        self.report["Averages"] = {"IOU" : np.sum(list(self.iourecord.values())) / len(self.iourecord)}
...
def configure_args():
    ...
    ap.add_argument("-f", "--folder", help="path to (optional) folder file", default="ball_images/")
    ap.add_argument("-o", "--output", help="path to (optional) report file", default="ball_images/report.json")
    ap.add_argument("-l", "--label", help="path to (optional) file of labels for images", default="ball_images/images.json")
